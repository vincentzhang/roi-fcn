...
class acce(imdb):
    def __init__(self, image_set, use_empty=False, devkit_path=None, vol=None):
        imdb.__init__(self, 'acce_' + image_set)
        self._image_set = image_set
        self._devkit_path = self._get_default_path() if devkit_path is None \
                            else devkit_path
        self._data_path = self._devkit_path
        # image_set = train or test
        # self._devkit_path: data/acce/
        # self._data_path  : data/acce/
        # train image: data/acce/train.txt
        # test image: data/acce/test.txt
        # bbox: data/acce/bbox
        # image: data/acce/resized_vols_2d.h5
        # pix-label: data/acce/resized_label_ac_2d.h5
        ...
        self._image_ext = '.h5'
        self._label_ext = '.h5'
        self._h5_name = 'cropped_seg_band' 
        # for training/ vol-specific testing
        #self._imagedb_name = os.path.join(self._data_path, self._h5_name+'.h5')
        # for overall testing
        self._imagedb_name = os.path.join(self._data_path,'resized_vols_2d.h5')
        # handle to the hdf5 file
        self._image_h5f = h5py.File(self._imagedb_name, 'r')
        self._label_h5f = h5py.File(os.path.join(self._data_path,'resized_label_ac_2d.h5'), 'r')
        # acce specific config options
        ...
        assert os.path.exists(self._devkit_path), \
                'acce path does not exist: {}'.format(self._devkit_path)
    ...
    def get_label(self, vol_name, idx):
        """ Binary label """
        label = np.asarray(self._label_h5f[vol_name+'_label'][idx, :,:], dtype='uint8')
        return label

    def get_bbox(self, i):
        """ Return a dictionary that contains bbox coordinates """
        return self._load_acce_annotation(self._image_index[i])
    ...
    def _load_image_set_index(self):
        ...
        for name in self._vol_names:
            bbox_file = os.path.join(self._data_path, 'bbox', name + '_bbox.txt')
    ...
    def _get_default_path(self):
        """
        Return the default path where acce is expected to be installed.
        """
        return os.path.join(cfg.DATA_DIR, 'acce')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_acce_annotation(index)
