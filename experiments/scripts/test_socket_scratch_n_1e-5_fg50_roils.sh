...
# example: ./experiments/scripts/test_socket_scratch_n_1e-5_fg50_roils.sh test all 4586 1 16 1
...
LOG="experiments/logs/eval_socket_scratch_n_1e-5_fg50_roils_${NET}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
for i in $(seq $START_EPOCH $END_EPOCH); 
  ...
  echo "ITER: ${ITER}"
  NET_FINAL="output/socket_scratch_end2end/socket_train/vgg16_socket_scratch_n_m_1e-5_fg50_roils_1e-05_iter_${ITER}.caffemodel"
