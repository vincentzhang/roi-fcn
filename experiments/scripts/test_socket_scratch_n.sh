...
# example: ./experiments/scripts/test_socket_scratch_n.sh train all 4586 1 16
# example: ./experiments/scripts/test_socket_scratch_n.sh test partial, partial means only use slices with positive labels
...
LOG="experiments/logs/eval_socket_scratch_n_1e-05_end2end_${NET}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
#for i in "${ITERATION_LIST[@]}"
  ...
  NET_FINAL="output/socket_scratch_end2end/socket_train/vgg16_detect_socket_scratch_n_m_1e-05_iter_70000.caffemodel"
...
time ./tools/test_net_seg.py --gpu ${GPU_ID} \
   ...
   --net ${NET_FINAL} \
   --imdb ${TEST_IMDB} \
   --cfg experiments/cfgs/socket_scratch_n_end2end.yml \
   --suffix ${ITER}
done
