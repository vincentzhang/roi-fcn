...
# example: ./experiments/scripts/test_ins_scratch_n_fg.sh test all 1553 1 30 0
...
LOG="experiments/logs/eval_ins_scratch_n_fg10_1e-05_end2end_${NET}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
for i in $(seq $START_EPOCH $END_EPOCH); 
  ...
  echo "ITER: ${ITER}"
  NET_FINAL="output/ins_scratch_end2end/ins_train/vgg16_detect_ins_scratch_n_m_fg10_1e-05_iter_${ITER}.caffemodel"
...
time ./tools/test_net_seg.py --gpu ${GPU_ID} \
   ...
   --net ${NET_FINAL} \
   --imdb ${TEST_IMDB} \
   --cfg experiments/cfgs/ins_scratch_n_fg10_end2end.yml \
   --suffix ${ITER}
done
