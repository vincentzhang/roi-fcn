...
# example: ./experiments/scripts/test_socket_scratch_n_fg_pre.sh train all 4586 1 16
...
LOG="experiments/logs/eval_socket_scratch_n_fg_pre_1e-05_end2end_${NET}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
#for i in "${ITERATION_LIST[@]}"
  ...
  #NET_FINAL="output/socket_scratch_end2end/socket_train/vgg16_detect_socket_scratch_n_m_fg_pre_1e-05_iter_${ITER}.caffemodel"
  NET_FINAL="output/socket_scratch_end2end/socket_train/vgg16_detect_socket_scratch_n_m_fg_pre_1e-05_iter_${ITER}.caffemodel"
