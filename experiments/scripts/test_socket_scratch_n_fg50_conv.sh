...
# example: ./experiments/scripts/test_socket_scratch_n_fg50_conv.sh test all 4586 1 1 1
...
LOG="experiments/logs/eval_socket_scratch_n_fg50_conv_1e-05_end2end_${NET}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/test_net_seg.py --gpu ${GPU_ID} \
   --def models/${PT_DIR}/${NET}/detect_end2end/test_conv.prototxt \
   --net ${NET_FINAL} \
   --imdb ${TEST_IMDB} \
