...
LOG="experiments/logs/eval_ins_scratch_n_fg50_1e-04_end2end_${NET}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
for i in $(seq $START_EPOCH $END_EPOCH); 
  ...
  echo "ITER: ${ITER}"
  NET_FINAL="output/ins_scratch_end2end/ins_train/vgg16_detect_ins_scratch_n_m_fg50_1e-04_iter_${ITER}.caffemodel"
