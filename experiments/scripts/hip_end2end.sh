#!/bin/bash
# Usage:
# ./experiments/scripts/hip_end2end.sh GPU NET DATASET [options args to {train,test}_net.py]
# DATASET is hip.
#
# Example:
# ./experiments/scripts/hip_end2end.sh 0 VGG16 hip\
...
LOG="experiments/logs/hip_end2end_${NET}_${EXTRA_ARGS_SLUG}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
